...
type AppNavbarProps = {
  variant?: "dashboard" | "create-sync"
  initialName?: string
  syncName?: string
  setSyncName?: (name: string) => void
}

export const AppNavbar = ({ variant = "dashboard" }: AppNavbarProps) => {
  const router = useRouter()

  const isDashboard = variant === "dashboard"

  const { syncName, setSyncName } = useSyncName()
    ...
    <div className="w-full border-b border-rose-600/40 font-red-hat-display">
      <div className="flex h-16 items-center justify-between px-4 tracking-wide text-white">
        <div className="font-semibold">
          {isDashboard ? "Your Syncs" : "Automation Flow Area"}
        </div>
        {/* BUTTONS */}

        {!isDashboard && (
          <div className="flex cursor-pointer items-center justify-center gap-1.25 rounded-xs px-4 py-2 font-semibold tracking-wide text-rose-100 transition-colors duration-200 hover:bg-rose-900/40 hover:text-white">
            <div className="flex items-center justify-center gap-1">
              <input
                value={syncName}
                onChange={(e) => setSyncName?.(e.target.value)}
                className="max-w-28 border-none bg-transparent text-center text-[14px] outline-none"
              />
              <span className="rounded-xs bg-blue-300 px-1 py-px text-[14px] text-black">
                Draft
              </span>
            </div>
            <FaAngleDown />
